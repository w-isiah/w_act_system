from apps.system import blueprint
...
def computer_info():
    ...
    return render_template(
        'system/computer_info.html',  # update this path if using a different template
...
@blueprint.route('/verify_key', methods=['POST'])
@csrf.exempt  # Only if CSRF is enabled globally
def verify_key():
    try:
        data = request.get_json(force=True)
        if not data or 'user_code' not in data:
            return jsonify({"status": "error", "message": "No activation key provided"}), 400

        user_code = data['user_code'].strip()
        if not user_code:
            return jsonify({"status": "error", "message": "Empty activation key"}), 400

        connection = get_db_connection()
        cursor = connection.cursor(dictionary=True)

        cursor.execute("SELECT * FROM activation_keys WHERE activation_key = %s", (user_code,))
        result = cursor.fetchone()

        if not result:
            return jsonify({"status": "invalid", "message": "Activation key not found"}), 404

        if result["mac_address"] or result["device_id"]:
            return jsonify({"status": "used", "message": "Activation key already used"}), 409
        ...
        print(f"[Server] ❌ Key verification failed: {e}")
...
@blueprint.route('/device_control')
def device_control():
    """
    Fetch system information records from the system_info table.
    Render the computer info management page.
    ...
    return render_template(
        'system/device_control.html',  # update this path if using a different template
        system_data=system_data,
        segment='device_control'
    )
...
def add_activation_key():
    ...
    if request.method == 'POST':
        ...
        if not subscription_plan_id:
            ...
            try:
                ...
                return redirect(url_for('system_blueprint.system'))
    ...
    return render_template("system/add_activation_key.html", plans=plans,activation_key=activation_key)
...
def edit_activation_key(key_id):
    ...
    return render_template(
        'system/edit_activation_key.html',
...
def route_template(template):
    ...
    try:
        ...
        return render_template("system/" + template, segment=segment)
